...
const sightseeingSearchTool = new DynamicStructuredTool({
  ...
  func: async ({ lat, long, searchQuery, radius = 10 }: { lat: number, long: number, searchQuery: string, radius?: number }) => {
    consola.info(`sightseeingSearchTool called with lat: ${lat}, long: ${long}, searchQuery: ${searchQuery}, radius: ${radius}`)
    ...
    consola.info('sightseeingURL', sightseeingURL.toString())
...
export const getSightseeingSearchTool = () => sightseeingSearchTool
