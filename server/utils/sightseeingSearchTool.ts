...
const sightseeingSearchTool = new DynamicStructuredTool({
  ...
  func: async ({ lat, long, searchQuery, radius = 10 }: { lat: number; long: number; searchQuery: string; radius?: number }) => {
    console.log(`sightseeingSearchTool called with lat: ${lat}, long: ${long}, searchQuery: ${searchQuery}, radius: ${radius}`)
    ...
    console.log('sightseeingURL', sightseeingURL.toString())
