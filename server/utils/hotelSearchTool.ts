...
const hotelSearchTool = new DynamicStructuredTool({
  ...
  func: async ({ lat, long, searchQuery, radius = 10 }: { lat: number, long: number, searchQuery: string, radius?: number }) => {
    consola.info(`hotelSearchTool called with lat: ${lat}, long: ${long}, searchQuery: ${searchQuery}, radius: ${radius}`)
    ...
    consola.info('locationSearchURL', locationSearchURL.toString())
